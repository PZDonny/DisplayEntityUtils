...
@Description("Get a persistent packet-based Group from its ID")
@Examples({
...
public class ExprPersistentPacketGroupFromId extends SimpleExpression<PacketDisplayEntityGroup> {
    ...
    public static void register(SyntaxRegistry registry){
        registry.register(SyntaxRegistry.EXPRESSION,
                SyntaxInfo.Expression.builder(ExprPersistentPacketGroupFromId.class, PacketDisplayEntityGroup.class)
                        .addPattern("persistent packet group (from|with) [global] id %string%")
                        .supplier(ExprPersistentPacketGroupFromId::new)
                        .build()
        );
    }
