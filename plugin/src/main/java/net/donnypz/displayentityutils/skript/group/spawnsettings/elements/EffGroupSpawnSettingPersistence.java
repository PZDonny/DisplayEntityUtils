...
public class EffGroupSpawnSettingPersistence extends Effect {
    ...
    public static void register(SyntaxRegistry registry){
        registry.register(SyntaxRegistry.EFFECT,
                SyntaxInfo.builder(EffGroupSpawnSettingPersistence.class)
                        .addPattern("(make|set) %groupspawnsetting% [to] [:not] persistent by default [with[:out] [chunk load] [persistence] overrid(e|ing)]")
                        .supplier(EffGroupSpawnSettingPersistence::new)
                        .build()
        );
    }
