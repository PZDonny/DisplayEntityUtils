...
@Description("Get, and optionally register, the nearest or all nearby non-packet groups within a location")
...
public class ExprSpawnedGroupNearLocation extends SimpleExpression<SpawnedDisplayEntityGroup> {
    ...
    public static void register(SyntaxRegistry registry){
        registry.register(SyntaxRegistry.EXPRESSION,
                SyntaxInfo.Expression.builder(ExprSpawnedGroupNearLocation.class, SpawnedDisplayEntityGroup.class)
                        .addPattern("(1¦all|2¦nearest) [r:registered] spawned[ |-]group[s] within %number% [block[s]] of %location%")
                        .supplier(ExprSpawnedGroupNearLocation::new)
                        .build()
        );
    }
