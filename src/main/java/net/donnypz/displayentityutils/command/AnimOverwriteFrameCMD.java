...
class AnimOverwriteFrameCMD implements SubCommand{
    ...
    public void execute(Player player, String[] args) {
        ...
        if (args.length < 5) {
            player.sendMessage(Component.text("/mdis anim overwriteframe <frame-id> <delay-in-ticks> <duration-in-ticks>", NamedTextColor.RED));
            return;
        }
        try {
            ...
            player.sendMessage(DisplayEntityPlugin.pluginPrefix + ChatColor.GREEN + "Successfully overwritten animation frame");
